...
type CurrencyCode = 'USD' | 'EUR' | 'GBP' | 'PKR' | 'SAR' | 'INR' | 'AED' | 'JPY'
...
export const REGIONS: Record<string, RegionConfig> = {
  PK: { code: 'PK', name: 'Pakistan', flagTag: '[PK]', timezone: 'Asia/Karachi', tzAbbrev: 'PKT', currency: 'PKR', calendarType: 'Gregorian / Hijri', city: 'Karachi' },
  US: { code: 'US', name: 'United States', flagTag: '[US]', timezone: 'America/New_York', tzAbbrev: 'EST/EDT', currency: 'USD', calendarType: 'Gregorian Standard', city: 'New York' },
  EU: { code: 'EU', name: 'Eurozone', flagTag: '[EU]', timezone: 'Europe/Paris', tzAbbrev: 'CET/CEST', currency: 'EUR', calendarType: 'Gregorian European', city: 'Paris' },
  UK: { code: 'UK', name: 'United Kingdom', flagTag: '[UK]', timezone: 'Europe/London', tzAbbrev: 'GMT/BST', currency: 'GBP', calendarType: 'Gregorian UK', city: 'London' },
  IN: { code: 'IN', name: 'India', flagTag: '[IN]', timezone: 'Asia/Kolkata', tzAbbrev: 'IST', currency: 'INR', calendarType: 'Gregorian / Saka', city: 'Mumbai' },
  SA: { code: 'SA', name: 'Saudi Arabia', flagTag: '[SA]', timezone: 'Asia/Riyadh', tzAbbrev: 'AST', currency: 'SAR', calendarType: 'Hijri / Islamic', city: 'Riyadh' },
  AE: { code: 'AE', name: 'United Arab Emirates', flagTag: '[AE]', timezone: 'Asia/Dubai', tzAbbrev: 'GST', currency: 'AED', calendarType: 'Gregorian / Hijri', city: 'Dubai' },
  JP: { code: 'JP', name: 'Japan', flagTag: '[JP]', timezone: 'Asia/Tokyo', tzAbbrev: 'JST', currency: 'JPY', calendarType: 'Gregorian / Japanese', city: 'Tokyo' },
}
...
export function getRegionalTimeInfo(timezone: string, date: Date = new Date()) {
  try {
    ...
    return {
      timeWithSec: timeFormatter.format(date),
      shortTime: shortTimeFormatter.format(date),
      dateStr: dateFormatter.format(date),
      fullDateStr: fullDateFormatter.format(date),
      isoDate: isoFormatter.format(date),
      monthYearStr: monthYearFormatter.format(date),
    }
  } catch {
    const now = date
    return {
...
interface AppContextType {
  ...
  regionCode: string
  setRegionCode: (r: string) => void
  region: RegionConfig
  regionalTime: ReturnType<typeof getRegionalTimeInfo>
  lang: Language
...
const AppCtx = createContext<AppContextType>({
  ...
  setRegionCode: () => {},
  region: REGIONS.PK,
  regionalTime: getRegionalTimeInfo('Asia/Karachi'),
  lang: 'en',
  setLang: () => {},
  t: (k: string) => k,
...
const ICON_MAP: Record<string, React.FC<any>> = {
  ...
  Heart, Home, GraduationCap, Plane, Zap, Shield, Gift, Briefcase,
  Target, DollarSign, CreditCard
}

function getBudgetIcon(iconName: string): React.FC<any> {
...
const defaultBudgets: Budget[] = [
  { id: 1, name: 'Overall Monthly Budget', limit: 3000, category: 'All', iconName: 'Wallet', color: '#2CC7A7' },
  { id: 2, name: 'Food & Dining', limit: 600, category: 'Food', iconName: 'Utensils', color: '#EF4444' },
  { id: 3, name: 'Rent & Housing', limit: 1200, category: 'Housing', iconName: 'Lock', color: '#60A5FA' },
  { id: 4, name: 'Shopping', limit: 400, category: 'Shopping', iconName: 'ShoppingBag', color: '#F97316' },
  { id: 5, name: 'Transport & Travel', limit: 300, category: 'Transport', iconName: 'Car', color: '#3B82F6' },
  { id: 6, name: 'Entertainment', limit: 200, category: 'Entertainment', iconName: 'Film', color: '#7C6FCD' },
]
...
type Transaction = {
  ...
  date: string
  isoDate: string
  exactEntryTimestamp: number
  exactEntryDateStr: string
  regionCode: string
  icon: React.FC<any>
  color: string
...
const threeDaysAgoMs = nowMs - 259200000
...
const initTransactions: Transaction[] = [
  {
    ...
    category: 'Food',
    amount: -8.50,
    date: 'Jul 27, 2026 · 09:30 AM',
    isoDate: '2026-07-27',
    exactEntryTimestamp: oneHourAgo,
  ...
  {
    id: 2,
    name: 'House Rent Bill',
    category: 'Housing',
    amount: -1200.00,
    date: 'Jul 25, 2026 · 10:00 AM',
    isoDate: '2026-07-25',
    exactEntryTimestamp: twoDaysAgoMs,
  ...
  },
  {
    ...
    isoDate: '2026-07-26',
    exactEntryTimestamp: yesterdayMs,
  ...
  },
  {
    ...
    category: 'Transport',
    amount: -18.40,
    date: 'Jul 26, 2026 · 08:40 PM',
    isoDate: '2026-07-26',
    exactEntryTimestamp: yesterdayMs + 14400000,
  ...
  },
  {
    ...
    isoDate: '2026-07-24',
    exactEntryTimestamp: threeDaysAgoMs,
...
const suggestions = [
  "Show Miscellaneous entry dates",
  "Current Regional Clock & Calendar",
  "Fixed vs Miscellaneous Breakdown",
  "Remaining Budget & Daily Spending",
  "Predict End-of-Month Expenses",
]

const initNotifications: NotificationItem[] = [
  ...
  { id: 4, type: 'warning', title: 'Unusual Transaction', desc: 'Large transaction detected', time: 'Yesterday', read: true },
...
function processAIMessage(
  ...
  const miscTx = transactions.filter(t => t.expenseType === 'miscellaneous' && t.amount < 0)
  const fixedTx = transactions.filter(t => t.expenseType === 'fixed' && t.amount < 0)
  
  const totalSpent = transactions.filter(t => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0)
  const miscTotal = miscTx.reduce((s, t) => s + Math.abs(t.amount), 0)
  const fixedTotal = fixedTx.reduce((s, t) => s + Math.abs(t.amount), 0)
  const remainingBudget = Math.max(0, monthlyBudgetLimit - totalSpent)

  // Query about Miscellaneous expenses / dates
  if (query.includes('misc') || query.includes('entry date') || query.includes('when') || query.includes('miscellaneous')) {
    if (miscTx.length === 0) {
      return `You currently have no Miscellaneous expenses recorded. When you add one, Trackify will log its exact entry date and timestamp in your active regional timezone (${region.tzAbbrev} - ${region.name}).`
    }
    const lastMisc = miscTx[0]
    const entriesSummary = miscTx.slice(0, 4).map(t => 
      `- ${t.name}: ${formatMoney(t.amount)} -- Entered on ${t.exactEntryDateStr || t.date}`
    ).join('\n')

    return `MISCELLANEOUS EXPENSES & ENTRY DATES:\n\n` +
      `You have ${miscTx.length} Miscellaneous expense(s) totaling ${formatMoney(miscTotal)}.\n\n` +
      `Most Recent Entry:\n"${lastMisc.name}" (${formatMoney(lastMisc.amount)}) entered on ${lastMisc.exactEntryDateStr || lastMisc.date}.\n\n` +
      `Exact Entry Timestamps:\n${entriesSummary}\n\n` +
      `Trackify logs every Miscellaneous entry with the exact calendar date, minute, and second in ${region.name} (${region.tzAbbrev}).`
  }

  // Query about Regional Time / Calendar / Clock
  if (query.includes('time') || query.includes('calendar') || query.includes('date') || query.includes('region') || query.includes('clock') || query.includes('today')) {
    return `REGIONAL CALENDAR & REAL-TIME CLOCK:\n\n` +
      `- Active Region: ${region.name} (${region.city})\n` +
      `- Live Local Time: ${regionalTime.timeWithSec} (${region.tzAbbrev})\n` +
      `- Regional Calendar Date: ${regionalTime.fullDateStr}\n` +
      `- Calendar System: ${region.calendarType}\n` +
      `- Timezone Identifier: ${region.timezone}\n\n` +
      `Trackify stays continuously synced with this region's clock so all your financial entries reflect the precise local entry date.`
  }

  // Query about Fixed vs Miscellaneous Breakdown
  if (query.includes('fixed') || query.includes('breakdown') || query.includes('split') || query.includes('category') || query.includes('compare')) {
    const fixedPct = totalSpent > 0 ? Math.round((fixedTotal / totalSpent) * 100) : 0
    const miscPct = totalSpent > 0 ? Math.round((miscTotal / totalSpent) * 100) : 0

    return `FIXED VS MISCELLANEOUS EXPENSE BREAKDOWN:\n\n` +
      `- Fixed Expenses (Rent, Subscriptions, Bills): ${formatMoney(fixedTotal)} (${fixedPct}% of spent)\n` +
      `- Miscellaneous Expenses (Food, Shopping, Ad-hoc): ${formatMoney(miscTotal)} (${miscPct}% of spent)\n` +
      `- Total Spent: ${formatMoney(totalSpent)} out of ${formatMoney(monthlyBudgetLimit)} monthly limit.\n\n` +
      `Recommendation: Keeping Miscellaneous expenses below 30% of total budget will help you save more every month.`
  }

  // Query about Budget / Savings / Predictions
  if (query.includes('budget') || query.includes('cut') || query.includes('saving') || query.includes('predict') || query.includes('remaining') || query.includes('left')) {
    const pctUsed = monthlyBudgetLimit > 0 ? Math.round((totalSpent / monthlyBudgetLimit) * 100) : 0
    return `BUDGET & SPENDING STATUS:\n\n` +
      `- Monthly Budget Limit: ${formatMoney(monthlyBudgetLimit)}\n` +
      `- Total Spent So Far: ${formatMoney(totalSpent)} (${pctUsed}% used)\n` +
      `- Remaining Budget: ${formatMoney(remainingBudget)}\n\n` +
      `Forecast: You have ${formatMoney(remainingBudget)} left for the rest of ${regionalTime.monthYearStr}. You are on track to stay within your budget.`
  }

  // Default fallback answer
  return `I have analyzed your financial notebook for ${region.name}:\n\n` +
    `- Total Spent: ${formatMoney(totalSpent)} (${miscTx.length} Miscellaneous entries, ${fixedTx.length} Fixed entries)\n` +
    `- Remaining Budget: ${formatMoney(remainingBudget)}\n` +
    `- Current Regional Date & Time: ${regionalTime.dateStr} at ${regionalTime.shortTime} (${region.tzAbbrev})\n\n` +
    `Feel free to ask me when any Miscellaneous expense was entered or to check your regional calendar.`
}

// ─── Shared Components ────────────────────────────────────────────────────────

function StatusBar() {
  const { theme, regionalTime, region } = useApp()
  const [timeStr, setTimeStr] = useState(() => regionalTime.shortTime)

  useEffect(() => {
    const interval = setInterval(() => {
      const info = getRegionalTimeInfo(region.timezone)
      setTimeStr(info.shortTime)
    }, 1000)
...
function OnboardingScreen({ onNext }: { onNext: () => void }) {
  ...
  const slides = [
    { icon: Wallet, title: 'Manage Multiple Budgets', desc: 'Create and track custom budget limits for Food, Travel, Rent, and Shopping with custom icons.' },
    { icon: Calendar, title: 'Regional Time & Calendar', desc: 'Stays updated with exact calendars and local times across Pakistan, US, Europe, Gulf, and Asia.' },
    { icon: Sparkles, title: 'AI Assistant Upgraded', desc: 'Ask AI about Miscellaneous entry dates, fixed budget splits, or local timezone forecasts.' },
  ]
      ...
      <div style={{ padding: '0 20px', marginTop: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
          {[
            ...
            { icon: Calendar, label: 'Calendar', action: () => onOpenModal('calendar') },
        ...
        <div style={{ backgroundColor: theme.surface, borderRadius: 8, border: `1px solid ${theme.border}`, overflow: 'hidden', boxShadow: theme.mode === 'light' ? '0 2px 12px rgba(0,0,0,0.06)' : 'none' }}>
          {transactions.length === 0 ? (
            ...
            transactions.slice(0, 5).map((tItem, i) => {
              const isMisc = tItem.expenseType === 'miscellaneous'
              return (
                <div key={tItem.id}>
...
function ManageBudgetModal({
  ...
  onClose: () => void
  budget?: Budget | null
  onSave: (data: Omit<Budget, 'id'> | Budget) => void
  theme: Theme
}) {
  ...
  const [limit, setLimit] = useState(budget ? String(budget.limit) : '500')
  const [category, setCategory] = useState(budget?.category || 'Custom')
  const [iconName, setIconName] = useState(budget?.iconName || 'Wallet')
  const [color, setColor] = useState(budget?.color || EMERALD)

  const availableIcons = [
    'Wallet', 'Utensils', 'ShoppingBag', 'Car', 'Lock', 'Film',
    'Dumbbell', 'Coffee', 'Heart', 'Home', 'GraduationCap', 'Plane',
  ...
  const handleSave = () => {
    ...
    if (budget) {
      onSave({ ...budget, name: name.trim(), limit: numLimit, category, iconName, color })
    } else {
      onSave({ name: name.trim(), limit: numLimit, category, iconName, color })
    }
    onClose()
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
      <div style={{ backgroundColor: theme.surface, borderRadius: '16px 16px 0 0', padding: '20px 20px 32px', width: '100%', maxWidth: 390, border: `1px solid ${theme.border}`, borderBottom: 'none', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }} className="scrollbar-thin">
          {/* Budget Name */}
          <div>
            <label style={{ fontSize: 11, color: theme.textSec, fontWeight: 700, letterSpacing: 0.5 }}>BUDGET NAME</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Groceries & Dining" style={{ width: '100%', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: 8, padding: '10px 14px', color: theme.text, fontSize: 14, marginTop: 6, outline: 'none' }} />
          </div>

          {/* Budget Limit */}
          <div>
            <label style={{ fontSize: 11, color: theme.textSec, fontWeight: 700, letterSpacing: 0.5 }}>MONTHLY LIMIT ({symbol.trim()})</label>
            <input type="number" value={limit} onChange={e => setLimit(e.target.value)} placeholder="500" style={{ width: '100%', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: 8, padding: '10px 14px', color: theme.text, fontSize: 16, fontWeight: 700, marginTop: 6, outline: 'none' }} />
          </div>

          {/* Category Dropdown */}
          <div>
            <label style={{ fontSize: 11, color: theme.textSec, fontWeight: 700, letterSpacing: 0.5 }}>CATEGORY</label>
            <select value={category} onChange={e => setCategory(e.target.value)} style={{ width: '100%', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: 8, padding: '10px 14px', color: theme.text, fontSize: 14, marginTop: 6, outline: 'none' }}>
          ...
          {/* Icon Selector Grid */}
          <div>
            <label style={{ fontSize: 11, color: theme.textSec, fontWeight: 700, letterSpacing: 0.5 }}>CHOOSE BUDGET ICON</label>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 6, marginTop: 8 }}>
              {availableIcons.map(icName => {
                const IconComponent = getBudgetIcon(icName)
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px' }}>
                        <button onClick={() => onSelectTx(t)} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 12, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <p style={{ fontSize: 10, color: theme.textSec, marginTop: 2 }}>{t.category} · {t.exactEntryDateStr || t.date}</p>
                          </div>
                        </button>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
...
function AddExpenseScreen({ onBack, onAdd }: { onBack: () => void; onAdd?: (t: Omit<Transaction, 'id'>) => void }) {
  const { theme, currency, region, regionalTime } = useApp()
  const symbol = currencyMap[currency]?.symbol || '$'
  ...
  const handleSave = () => {
    ...
    if (val > 0 && onAdd) {
      const nowInfo = getRegionalTimeInfo(region.timezone)
      const exactDateStr = `${nowInfo.dateStr} at ${nowInfo.timeWithSec} (${region.tzAbbrev})`

      onAdd({
        name: note.trim() || `${category} (${expenseType === 'fixed' ? 'Fixed' : 'Misc'})`,
        category,
        amount: -val,
        date: `${nowInfo.dateStr} · ${nowInfo.shortTime}`,
        isoDate: nowInfo.isoDate,
        exactEntryTimestamp: Date.now(),
        exactEntryDateStr: exactDateStr,
      ...
      <div style={{ padding: '0 20px', marginTop: 4 }}>
        <div style={{ backgroundColor: 'rgba(44,199,167,0.1)', borderRadius: 6, padding: '6px 12px', border: `1px solid ${EMERALD}30`, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: theme.text, fontWeight: 600 }}>
            Exact Entry Timestamp: {regionalTime.dateStr} at {regionalTime.shortTime} ({region.tzAbbrev})
          </span>
        </div>
      </div>
...
function AIScreen({
  ...
  const { theme, t, currency, region, regionalTime, formatMoney } = useApp()
  
  const initialAIMessage = {
    role: 'ai',
    content: `Hi! I'm your updated Trackify AI Advisor. I am synced live with your active regional calendar (${region.flagTag} ${region.name} - ${region.tzAbbrev}) and track exact entry dates for your Miscellaneous expenses. How can I help you today?`
  }
  ...
  const sendText = (textToSend: string) => {
    ...
    const aiReply = processAIMessage(
      txt,
      transactions,
      monthlyBudgetLimit,
      currency,
    ...
    setMessages(m => [...m, { role: 'user', content: txt }, { role: 'ai', content: aiReply }])
    setInput('')
      ...
      <div ref={scrollRef} style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {messages.length === 0 ? (
          <div style={{ margin: 'auto', textAlign: 'center', padding: '24px 16px' }}>
            ...
            <p style={{ fontSize: 12, color: theme.textSec, marginTop: 4 }}>Ask about your Miscellaneous entry dates, regional clocks, or budget advice!</p>
          </div>
        ) : (
          messages.map((m, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', alignItems: 'flex-start', gap: 8 }} className="animate-fade-in">
              ...
              <div style={{ maxWidth: '82%', backgroundColor: m.role === 'user' ? CARD_DARK_SOLID : theme.surface, color: m.role === 'user' ? '#FFFFFF' : theme.text, borderRadius: 8, padding: '10px 14px', fontSize: 13, lineHeight: 1.6, border: m.role === 'user' ? '1px solid rgba(44,199,167,0.3)' : `1px solid ${theme.border}`, whiteSpace: 'pre-line' }}>
                {m.content}
              </div>
            </div>
          ))
  ...
  return (
    <div style={{ backgroundColor: theme.bg, flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ padding: '16px 20px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {unreadCount > 0 && <Tag>{unreadCount} New</Tag>}
          {notifs.length > 0 && (
            <button
              onClick={onClearAll}
              style={{
      ...
      <div style={{ padding: '0 20px', marginTop: 16, paddingBottom: 24, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {notifs.length === 0 ? (
          <div style={{ backgroundColor: theme.surface, borderRadius: 8, padding: '32px 20px', textAlign: 'center', border: `1px solid ${theme.border}` }}>
            <Bell size={32} color={theme.textSec} style={{ margin: '0 auto 12px', opacity: 0.5 }} />
            <p style={{ color: theme.text, fontSize: 14, fontWeight: 600 }}>No notifications</p>
            <p style={{ color: theme.textSec, fontSize: 12, marginTop: 4 }}>You are all caught up!</p>
          </div>
        ) : (
          notifs.map(n => {
            const NIcon = typeIcons[n.type] || Bell
            const color = typeColors[n.type] || EMERALD
            return (
...
function CurrencyModal({ onClose, selectedRegion, onSelectRegion, theme }: { onClose: () => void; selectedRegion: string; onSelectRegion: (r: string) => void; theme: Theme }) {
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ backgroundColor: theme.surface, borderRadius: '12px 12px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: 390, border: `1px solid ${theme.border}`, borderBottom: 'none', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexShrink: 0 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: theme.textSec }}>Select Region & Timezone for Live Calendar</p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'auto', flex: 1 }} className="scrollbar-thin">
          {Object.values(REGIONS).map(r => {
            const isSel = selectedRegion === r.code
            const liveT = getRegionalTimeInfo(r.timezone)
            return (
              <button key={r.code} onClick={() => { onSelectRegion(r.code); onClose() }} style={{ padding: '12px 16px', borderRadius: 8, backgroundColor: isSel ? 'rgba(44,199,167,0.12)' : theme.inputBg, border: `1px solid ${isSel ? EMERALD : theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: isSel ? EMERALD : theme.text, backgroundColor: 'rgba(44,199,167,0.15)', padding: '3px 6px', borderRadius: 4 }}>{r.flagTag}</span>
                  <div style={{ textAlign: 'left' }}>
                    <p style={{ fontWeight: 700, fontSize: 14, color: isSel ? EMERALD : theme.text }}>{r.name}</p>
                    <p style={{ fontSize: 10, color: theme.textSec }}>{r.currency} ({currencyMap[r.currency]?.symbol.trim()}) · {r.tzAbbrev}</p>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: isSel ? EMERALD : theme.textSec, fontFamily: 'monospace' }}>{liveT.shortTime}</span>
                  <p style={{ fontSize: 9, color: theme.textSec }}>{r.city}</p>
                </div>
              </button>
            )
          })}
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ backgroundColor: theme.surface, borderRadius: '12px 12px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: 390, border: `1px solid ${theme.border}`, borderBottom: 'none' }}>
        ...
        <p style={{ textAlign: 'center', fontSize: 13, color: theme.textSec, marginTop: 8, lineHeight: 1.5 }}>
          Warning: This will permanently erase all custom transactions, budgets, AI history, and restore default settings. This action cannot be undone.
        </p>
        <div className="flex gap-3 mt-6">
...
function TransactionDetailModal({
  ...
  const isMisc = transaction.expenseType === 'miscellaneous'
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              ...
              <span style={{ fontSize: 11, color: theme.textSec }}>{transaction.category}</span>
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: theme.textSec, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Close</button>
        </div>

        <div style={{ backgroundColor: theme.inputBg, borderRadius: 10, padding: '16px', border: `1px solid ${theme.border}`, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontSize: 12, color: theme.textSec }}>Amount</span>
            <span style={{ fontSize: 22, fontWeight: 800, color: transaction.amount > 0 ? SUCCESS : theme.text }}>{formatMoney(transaction.amount)}</span>
          </div>

          <div style={{ height: 1, backgroundColor: theme.divider, margin: '10px 0' }} />

          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: theme.textSec }}>Expense Type</span>
              <Tag variant={isMisc ? 'warning' : isFixed ? 'default' : 'success'}>
                {isMisc ? 'Miscellaneous' : isFixed ? 'Fixed Expense' : 'Income / Credit'}
              </Tag>
            </div>

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginTop: 4 }}>
              <span style={{ fontSize: 12, color: theme.textSec, flexShrink: 0 }}>Exact Entry Date</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: EMERALD, textAlign: 'right', maxWidth: 200, lineHeight: 1.4 }}>
                {transaction.exactEntryDateStr || transaction.date}
              </span>
            </div>

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: theme.textSec }}>Calendar ISO Date</span>
              <span style={{ fontSize: 12, fontWeight: 600, color: theme.text }}>{transaction.isoDate || '2026-07-27'}</span>
            </div>

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: theme.textSec }}>Region Code</span>
              <span style={{ fontSize: 12, fontWeight: 600, color: theme.text }}>{REGIONS[transaction.regionCode]?.flagTag || '[PK]'} {REGIONS[transaction.regionCode]?.name || 'Pakistan'}</span>
            </div>
          </div>
        </div>

        {isMisc && (
          <div style={{ backgroundColor: 'rgba(44,199,167,0.1)', borderRadius: 8, padding: '12px', border: `1px solid ${EMERALD}40`, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <Sparkles size={16} color={EMERALD} style={{ flexShrink: 0, marginTop: 2 }} />
            <p style={{ fontSize: 11, color: theme.text, lineHeight: 1.4 }}>
              <strong>Miscellaneous Entry Recorded:</strong> Trackify stays synced with regional calendars and logged this item on exact local time ({transaction.exactEntryDateStr || transaction.date}).
            </p>
          </div>
        )}
      </div>
...
function RegionalCalendarModal({
  onClose,
  regionCode,
  onSelectRegion,
  transactions,
  ...
  const currentRegion = REGIONS[regionCode] || REGIONS.PK
  const [liveInfo, setLiveInfo] = useState(() => getRegionalTimeInfo(currentRegion.timezone))
  const [selectedDate, setSelectedDate] = useState<string>(liveInfo.isoDate)

  useEffect(() => {
    const timer = setInterval(() => {
      setLiveInfo(getRegionalTimeInfo(currentRegion.timezone))
    }, 1000)
    return () => clearInterval(timer)
  }, [currentRegion.timezone])

  const daysInMonth = 31
  const startDayOffset = 3
  const daysArray = Array.from({ length: daysInMonth }, (_, i) => i + 1)

  const monthYearPrefix = liveInfo.isoDate.slice(0, 7)
  const dateExpensesMap: Record<string, Transaction[]> = {}
  transactions.forEach(t => {
    const iso = t.isoDate || liveInfo.isoDate
    if (!dateExpensesMap[iso]) dateExpensesMap[iso] = []
    dateExpensesMap[iso].push(t)
  })

  const selectedDayTx = dateExpensesMap[selectedDate] || []
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
      <div style={{ backgroundColor: theme.surface, borderRadius: '16px 16px 0 0', padding: '20px 20px 32px', width: '100%', maxWidth: 390, border: `1px solid ${theme.border}`, borderBottom: 'none', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Calendar size={20} color={EMERALD} />
            <div>
              <p style={{ fontWeight: 700, fontSize: 16, color: theme.text }}>Regional Calendar & Clock</p>
              <p style={{ fontSize: 10, color: theme.textSec }}>Synced Live with Regional Timezones</p>
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: theme.textSec, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Done</button>
        </div>
        ...
        {/* Region Selector Bar */}
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 8, marginBottom: 12, flexShrink: 0 }} className="scrollbar-thin">
          {Object.values(REGIONS).map(r => (
            <button
              key={r.code}
              onClick={() => { onSelectRegion(r.code); setSelectedDate(getRegionalTimeInfo(r.timezone).isoDate) }}
              style={{
                padding: '6px 10px', borderRadius: 8, flexShrink: 0,
                backgroundColor: regionCode === r.code ? EMERALD : theme.inputBg,
                color: regionCode === r.code ? '#081A18' : theme.text,
                border: `1px solid ${regionCode === r.code ? EMERALD : theme.border}`,
        ...
        {/* Live Regional Clock Banner */}
        <div style={{ backgroundColor: CARD_DARK_SOLID, borderRadius: 10, padding: '14px 16px', border: `1px solid ${EMERALD}40`, marginBottom: 14, flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 11, fontWeight: 800, color: EMERALD }}>{currentRegion.flagTag}</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#FFFFFF' }}>{currentRegion.name} ({currentRegion.city})</span>
              </div>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)', marginTop: 2 }}>{liveInfo.fullDateStr}</p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontSize: 18, fontWeight: 800, color: EMERALD, letterSpacing: 0.5, fontFamily: 'monospace' }}>{liveInfo.timeWithSec}</p>
              <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>{currentRegion.tzAbbrev} Time</span>
            </div>
          </div>
        </div>

        {/* Interactive Calendar Grid */}
        <div style={{ backgroundColor: theme.inputBg, borderRadius: 10, padding: '12px', border: `1px solid ${theme.border}`, marginBottom: 14, flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <span style={{ fontWeight: 700, fontSize: 13, color: theme.text }}>{liveInfo.monthYearStr}</span>
            <span style={{ fontSize: 10, color: EMERALD, fontWeight: 700, backgroundColor: 'rgba(44,199,167,0.12)', padding: '3px 8px', borderRadius: 4 }}>
              {currentRegion.calendarType}
            </span>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, textAlign: 'center', marginBottom: 6 }}>
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
              <span key={i} style={{ fontSize: 10, fontWeight: 700, color: theme.textSec }}>{d}</span>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
            ...
            {daysArray.map(dayNum => {
              ...
              const iso = `${monthYearPrefix}-${dayStr}`
              const isToday = iso === liveInfo.isoDate
              const isSelected = iso === selectedDate
              const txList = dateExpensesMap[iso] || []
              const hasMisc = txList.some(t => t.expenseType === 'miscellaneous')
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isSelected ? EMERALD : isToday ? 'rgba(44,199,167,0.2)' : theme.surface,
                    color: isSelected ? '#081A18' : isToday ? EMERALD : theme.text,
                ...
                >
                  ...
                  {txList.length > 0 && (
                    <div style={{ display: 'flex', gap: 2, position: 'absolute', bottom: 3 }}>
                      <div style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: isSelected ? '#081A18' : hasMisc ? WARNING : EMERALD }} />
                    </div>
                  )}
                </button>
              )
        ...
        <div style={{ overflowY: 'auto', flex: 1 }} className="scrollbar-thin">
          <p style={{ fontSize: 11, fontWeight: 700, color: theme.textSec, marginBottom: 8 }}>
            EXPENSES ON {selectedDate} ({selectedDayTx.length} items)
          </p>
          {selectedDayTx.length === 0 ? (
            <p style={{ fontSize: 12, color: theme.textSec, textAlign: 'center', padding: '16px 0' }}>
                ...
                <div key={tx.id} style={{ backgroundColor: theme.inputBg, borderRadius: 8, padding: '10px 12px', border: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 9, color: theme.textSec }}>
                        {tx.expenseType === 'miscellaneous' ? 'Misc' : 'Fixed'} · Entered {tx.exactEntryDateStr ? tx.exactEntryDateStr.split(' at ')[1] : tx.date}
                      </p>
                    </div>
                  </div>
                  <span style={{ fontSize: 12, fontWeight: 700, color: theme.text }}>${Math.abs(tx.amount).toFixed(2)}</span>
                </div>
              ))}
            </div>
          )}
  ...
  const groups = [
    {
      title: 'Regional Time & Calendar', items: [
        ...
        { label: `Timezone: ${region.tzAbbrev} (${region.city})`, icon: Globe, toggle: false, value: false, onToggle: () => onOpenModal('currency') },
      ]
    },
    {
    ...
    },
    {
      title: t('exportData'), items: [
        ...
        { label: 'Share Summary', icon: Share2, toggle: false, value: false, onToggle: () => onOpenModal('share') },
    ...
    <div style={{ backgroundColor: theme.bg, flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ padding: '0 20px', marginTop: 16, paddingBottom: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ backgroundColor: theme.surface, borderRadius: 8, padding: '16px', border: `1px solid ${theme.border}`, boxShadow: theme.mode === 'light' ? '0 2px 10px rgba(0,0,0,0.05)' : 'none' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <button onClick={() => isDark && toggleTheme()} style={{ padding: '12px 14px', borderRadius: 6, border: `1.5px solid ${!isDark ? EMERALD : theme.border}`, backgroundColor: !isDark ? 'rgba(44,199,167,0.08)' : theme.inputBg, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
                <Sun size={16} color={!isDark ? EMERALD : theme.textSec} />
              ...
              <button onClick={() => !isDark && toggleTheme()} style={{ padding: '12px 14px', borderRadius: 6, border: `1.5px solid ${isDark ? EMERALD : theme.border}`, backgroundColor: isDark ? 'rgba(44,199,167,0.12)' : theme.inputBg, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
                <Moon size={16} color={isDark ? EMERALD : theme.textSec} />
        ...
        <div style={{ backgroundColor: theme.surface, borderRadius: 8, border: `1px solid ${theme.border}`, padding: '16px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 11, color: theme.textSec }}>{t('appVersion')}</p>
        </div>
      </div>
    </div>
  )
...
export default function App() {
  ...
  const [regionCode, setRegionCode] = useState<string>('PK')
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setRegionalTime(getRegionalTimeInfo(region.timezone))
    }, 1000)
    return () => clearInterval(timer)
  }, [region.timezone])
  ...
  const handleSelectRegion = (code: string) => {
    setRegionCode(code)
    const newReg = REGIONS[code]
    if (newReg) {
      setCurrency(newReg.currency)
      setRegionalTime(getRegionalTimeInfo(newReg.timezone))
    }
  }
  ...
  const handleClearAllNotifs = () => {
    ...
    setToastMsg("All notifications cleared")
    setTimeout(() => setToastMsg(null), 2000)
  }
  ...
  const handleSaveBudget = (data: Omit<Budget, 'id'> | Budget) => {
    if ('id' in data) {
      setBudgets(prev => prev.map(b => b.id === data.id ? data : b))
      setToastMsg("Budget updated")
    } else {
  ...
  const hasUnreadNotifs = notificationsList.some(n => !n.read) || notificationsList.length > 0
  ...
  return (
    <AppCtx.Provider value={{
      ...
      setCurrency,
      regionCode,
      setRegionCode: handleSelectRegion,
      region,
      regionalTime,
      lang,
      setLang,
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {activeModal === 'currency' && <CurrencyModal onClose={() => setActiveModal(null)} selectedRegion={regionCode} onSelectRegion={handleSelectRegion} theme={theme} />}
